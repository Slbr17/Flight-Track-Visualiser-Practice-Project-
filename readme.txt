...
Again: this is **just a personal practice project** to play with scraping, data cleaning, mapping, and basic GIS, *not* an official FlightAware tool.

---

## Why AeroAPI instead of scraping?

FlightAware provides an official, documented API called **AeroAPI** for programmatic access to flight data.

If you are building **anything real** (a tool, an app, a dashboard, research, etc.), you should:

- Use AeroAPI instead of scraping HTML.
- Follow FlightAware’s **Terms of Use** and **rate limits**.
- Handle authentication, quotas, and support via the official channels.

Scraping HTML pages can be:

- Fragile (any layout change breaks your scraper).
- Against a site’s **Terms of Service**.
- Impossible to scale or support.

This repository exists only as a **learning example** and should not be treated as a template for production work.
...
```bash
pip install playwright pandas openpyxl folium geopandas shapely
